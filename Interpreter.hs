...
interpretCmd (Fd x) (State {pen, pos, ang}) = State {pen, pos = computeNewPosition x ang pos, ang}
interpretCmd (Lt x) (State {pen, pos, ang = ang0}) = State {pen, pos, ang = ang0 + (pi * 2) * (fromIntegral x / 360)}
interpretCmd (Rt x) (State {pen, pos, ang = ang0}) = State {pen, pos, ang = ang0 - (pi * 2) * (fromIntegral x / 360)}
...
interpretProg (Cmd c) s = interpretCmd c s
interpretProg (Seq c p) s =
  let t = interpretProg (Cmd c) s
   in interpretProg p t
